				...
				for(x = pl->minx; x <= pl->maxx; x++)
				{
					...
					if(dc_yl <= dc_yh)
					{
						...
						angle = (viewangle+xtoviewangle[x])
							>>ANGLETOSKYSHIFT;
						yoffset = SKYTEXTUREMIDSHIFTED+((dc_yl-centery)*SCREENHEIGHT)/sysvideo.height;
						source = R_GetColumn(skyTexture, angle+offset)+yoffset;
						source2 = R_GetColumn(skyTexture2,angle+offset2)+yoffset;
						...
						if (detailshift) {
							do
							{
								if(source[frac>>16])
								{
									color = source[frac>>16];
									*dest++ = color;
									*dest++ = color;
								}
								else
								{
									color = source2[frac>>16];
									*dest++ = color;
									*dest++ = color;
								}
								frac += fracstep;
